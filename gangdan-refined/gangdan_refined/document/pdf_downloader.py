...
from ..research.models import DownloadResult, PaperMetadata, PaperRecord
...
class PDFDownloadManager:
    ...
    def discover_oa_urls(self, paper: PaperMetadata) -> List[Dict[str, str]]:
        ...
        if paper.doi:
            ...
            urls = self._discover_from_pmc_converter(paper.doi)
            for u in urls:
                if u["url"] not in seen_urls:
                    seen_urls.add(u["url"])
                    candidates.append(u)
    ...
    def _discover_from_unpaywall(self, doi: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            params = {"email": CONFIG.unpaywall_email or "gangdan@localhost"}
            proxies = get_proxies()
    ...
    def _discover_from_pmc_converter(self, doi: str) -> List[Dict[str, str]]:
        """Discover OA URL from PMC ID Converter (CiteBox approach #3).
        ...
        try:
            params = {
                ...
                "format": "json",
                "tool": "gangdan",
                "email": CONFIG.unpaywall_email or "gangdan@localhost",
            }
            proxies = get_proxies()
            resp = self._session.get(
                PMC_ID_CONVERTER_URL, params=params, timeout=15, proxies=proxies
            )
            resp.raise_for_status()
            data = resp.json()
            ...
            for record in records:
                ...
                if pmcid:
                    results.append({
                        ...
                        "source": "pmc_converter",
                    })
        except Exception as e:
            logger.debug("[PMC Converter] DOI %s failed: %s", doi, e)
