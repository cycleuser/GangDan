...
class TestCLIChatCommand:
    ...
    def test_chat_no_model_configured(self, temp_data_dir):
        ...
        # Manually set to empty to test the validation
        ...
        # Should return 1 when no model is configured (manual override)
        assert result == 1
...
class TestCLIAICommand:
    ...
    def test_ai_no_model(self, temp_data_dir):
        ...
        # Set chat_model to empty to trigger the "no model" error
        config_module.CONFIG.chat_model = ""
        ...
        # Should return 1 when no model is configured
        assert result == 1
